...
def main(args):
    ...
    out_dir = out_dir + "/data/bam/"
    subprocess.call ('mkdir -p ' + out_dir, shell=True)

    fout = sh_dir + 'STAR_rnaalign_' + start + '_' + end + '.sh'
    sh = open (fout, 'w')
    pts = open (pt_file)  # note there is a header
    log = log_dir + 'STAR_rnaalign_' + start + '_' + end + '.log'
    lns = pts.readlines ()
    ...
    for i in range ((int) (start), (int) (end) + 1):
        ...
        R1 = inputdir + sample_id + '_R1.fq.gz'
        R2 = inputdir + sample_id + '_R2.fq.gz'

        cmd = 'echo starting alignment at time `date` \n\n'
        cmd += 'echo 1.Running STAR commandline on patient ' + sample_id + '...\n\n'

        cmd += STAR + ' --runThreadN 8 --genomeDir ' + genomeindex + ' --readFilesIn ' + R1 + ' ' + R2 + ' '
        cmd += '--readFilesCommand zcat '
        cmd += '--outFileNamePrefix ' + out_dir + '/' + sample_id + '.'
        ...
        cmd +='--outSAMattrRGline ID:' + sample_id + ' PL: illumina PU: CCD LIB:' + sample_id +' SM:' + sample_id + ' \n'

        cmd += 'echo $(date) STAR is done. \n\n'

        cmd += 'echo $(date) Start bam sort for HTSeq-count \n\n'

        cmd += SAMTOOLS + ' sort -@ 8 -n ' + out_dir + '/' + sample_id + '.Aligned.out.bam -o ' + out_dir + '/' + sample_id+ '_name_sorted.bam \n'

        cmd += 'echo $(date) Start bam sort for cufflinks \n\n'

        cmd += SAMTOOLS + ' sort -@ 8 ' + out_dir + '/' + sample_id + '.Aligned.out.bam -o ' + out_dir + '/' + sample_id + '_sorted.bam & wait; \n'
        cmd += SAMTOOLS + ' index -@ 8 ' + out_dir + '/' + sample_id + '_sorted.bam & wait; \n'

        cmd += 'echo Finished STAR \n\n'
        sh.write (cmd)
    sh.close ()
    pts.close ()
...
'''
python /data/cyu/scripts/rna/STAR.py \
-i /data/gbm/rna/ -p /data/cyu/gbm_rna/samples.txt -s 3 -e 3 -o /data/cyu/gbm_rna/
