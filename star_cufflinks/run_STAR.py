...
def parse_arguments():
	...
	parser.add_argument ('-f', dest='flag', help='annotate with raw_fq or trim_fq')
...
def main():
	...
	fout = sh_dir + 'STAR_' + flag +'_'+ start + '_' + end + '.sh'
	sh = open (fout, 'w')
	sh.write ("Run STAR for RNA-Seq in version STAR-2.7.5a")
	sh.write("\n")
	pts = open (pt_file)  # note there is a header
	log = log_dir + 'STAR_' + flag + '_'+ start + '_' + end + '.log'
	lns = pts.readlines ()
	cmd = 'echo \'###########################################\' >>' + log + ' 2>&1 &'
	cmd += '\n'
	...
	cmd += 'echo \'###########################################\' >>' + log + ' 2>&1 &'
	cmd += '\n\n\n'
	out_dir1 = outdir + '/data/STAR_' + flag + '/'
	...
	for i in range ((int) (start)-1, (int) (end)):
		...
		cmd +='Start Samtools \n\n'
		cmd += SAMTOOLS + ' view -bS ' + out_dir1 + '/' + pID + '.Aligned.out.sam > ' + out_dir1 + '/' + pID + '.Aligned.out.bam \n'
		...
		cmd += SAMTOOLS + ' sort -@ 8 ' + out_dir1 + '/' + pID + '.Aligned.out.bam -o ' + out_dir1 + '/' + pID+ '.sorted.bam \n'
...
-s 1 -e 10 -f raw_fq -o /data/cyu/RNA_Bea/

python /data/cyu/scripts/rna/run_STAR.py \
-i /data/cyu/RNA_Bea/data/trim_fq/ \
-p /data/cyu/RNA_Bea/sample.txt \
-s 1 -e 10 -f trim_fq -o /data/cyu/RNA_Bea/
